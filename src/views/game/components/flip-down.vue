  ...
  methods: {
    ...
    start(isFirst) {
      ...
      this.timer = setTimeout(() => {
        this.count++;
        const offset =
          new Date().getTime() - (this.startTime + this.count * 1000); // 计算出当前执行倒计时的时间与实际应执行时间的偏差，
        this.nextTime = 1000 - offset; //进而可以计算出下次执行倒计时的时间。
        if (this.nextTime < 0) {
          this.nextTime = 0;
        }
        // console.log(`误差：${offset} ms，下一次执行：${this.nextTime} ms 后`);
